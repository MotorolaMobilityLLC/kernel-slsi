...
struct modem_ctl {
	...
	unsigned int mbx_ap_mif_freq;

	/* for tmu request */
	unsigned int gpio_tmu_req;
	unsigned int mbx_tmu_req;
	int irq_tmu_req;
	...
	unsigned int gpio_flm_uart_sel_rev06;
#endif
	...
	unsigned int gpio_fpga_cs_n;
#endif

#if defined(CONFIG_MACH_U1_KOR_LGT)
	unsigned int gpio_cp_reset_msm;
	unsigned int gpio_boot_sw_sel;
	void (*vbus_on)(void);
	void (*vbus_off)(void);
	bool usb_boot;
#endif
	...
	unsigned int gpio_sim_io_sel;
	unsigned int gpio_cp_ctrl1;
	unsigned int gpio_cp_ctrl2;
#endif
