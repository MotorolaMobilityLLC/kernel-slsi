...
static struct mbulk *hip4_skb_to_mbulk(struct hip4_priv *hip, struct sk_buff *skb, bool ctrl_packet)
{
	...
	if (fapi_is_ma(skb)) {
		SLSI_MBULK_COLOUR_SET(colour,
			(fapi_get_vif(skb)),
			(fapi_get_u16(skb, u.ma_unitdata_req.peer_index)),
...
int scsc_wifi_transmit_frame(struct slsi_hip4 *hip, bool ctrl_packet, struct sk_buff *skb)
{
	...
	if (ctrl_packet) {
		...
		SCSC_HIP4_SAMPLER_VIF_PEER(hip->hip_priv->minor, 1, fapi_get_vif(skb), fapi_get_u16(skb, u.ma_unitdata_req.peer_index));
	}
#endif
#ifdef CONFIG_SCSC_WLAN_DEBUG
